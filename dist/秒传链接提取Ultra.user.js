 ...
 * @LastEditTime: 2023-04-25 12:19:33
 * @LastEditors: mengzonefire
...
function ajax(config, callback, failback) {
    GM_xmlhttpRequest(__assign(__assign({}, config), { onload: function (r) {
            ...
            if (Math.floor(r.status / 100) === 2) {
                console.info("%s version: %s 接口返回: %s", TAG, const_version, JSON.stringify(r.response)); // user debug
                callback(r);
            }
            else
                failback(r.status);
 ...
 * @LastEditTime: 2023-04-25 12:17:40
 * @LastEditors: mengzonefire
